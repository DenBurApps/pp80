...
public class ScrollMechanic : MonoBehaviour, IDropHandler, IDragHandler, IBeginDragHandler, IPointerExitHandler, IPointerEnterHandler {
    ...
    private void Start() {
        camera = Camera.main;
        targetCanvas = BaseCanvas.canvas.gameObject.GetComponent<RectTransform>();
    ...
    {
        return contentTarget.GetChild(currentCenter).GetChild(0).GetComponent<TextMeshProUGUI>().text;
    }
    private void Update() {
